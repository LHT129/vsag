...
int OptQuery(vsag::DatasetPtr query, std::shared_ptr<vsag::Index> index, int k, int upperbound) {
    ...
    auto search_parameters = fmt::format(search_parameters_json, upperbound);
    auto anngt = *index->KnnSearch(query, k, search_parameters);
    std::unordered_set<int32_t> gt;
    for (int i = 0; i < k; i++) {
        gt.insert((int32_t) anngt->GetIds()[i]);
    }
    auto Cmp = [&](vsag::DatasetPtr cur) -> bool {
        std::unordered_set<int32_t> res;
        for (int i = 0; i < k; i++) {
            res.insert((int32_t) cur->GetIds()[i]);
        }
        for (const auto &item: gt) {
            if (res.find(item) == res.end()) {
                return false;
            }
        }
        return true;
    ...
    auto result = end;
    while (begin < end) {
        auto tmp = (begin + end) / 2;
        search_parameters = fmt::format(search_parameters_json, tmp);
        auto cur = *index->KnnSearch(query, k, search_parameters);
        if (Cmp(cur)) {
            result = end;
            end = tmp - 1;
        } else {
...
//    logger->Info(fmt::format("sq: {}, time: {}", sq_num_bits, result->first));
    ...
    for (auto ef_search: efs) {
        ...
        for (int i = 0; i < query_npts * 10; i++) {
            auto single_query = vsag::Dataset::Make();
            single_query->NumElements(1)->Dim(expected_dim)->Owner(false);
            double time_cost = 0;
            vsag::DatasetPtr ann_result;
            single_query->Float32Vectors(base->GetFloat32Vectors() + i * expected_dim);
...
//                ann_result = *index->KnnSearch(single_query, k, search_parameters);
//            }
//            std::cout << "search cost: " << time_cost << std::endl;
