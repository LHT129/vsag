...
Pyramid::repair_duplicate_connectivity(IndexNode& node,
    ...
    for (const auto source : available) {
        ...
        if (row.size() >= degree) {
            ...
            CHECK_ARGUMENT(replace != absent,
                           "Pyramid return bridge must preserve the forward tree");
            row[replace] = ids[target];
        } else {
            row.push_back(ids[target]);
        }
    ...
    if (return_bridges != 0) {
        logger::info("[pyramid_build_cache] repaired {} representative return bridges",
                     return_bridges);
    }
    if (bridges != 0) {
        logger::info("[pyramid_build_cache] repaired {} representative connectivity bridges",
                     bridges);
    }
}
...
Pyramid::add_graph_point(const Hierarchy& hierarchy,
    ...
    if (support_duplicate_ && not use_self_as_entry) {
            ...
            if (node.duplicate_admission_ == nullptr) {
                ...
                if (node.duplicate_admission_ == nullptr) {
                    ...
                    for (const auto id : node.get_ids_unlocked()) {
                        ...
                        if (admission->stripes[existing.first % 64]
                    ...
                    if (node.graph_param_->graph_storage_type_ !=
                        ...
                        for (InnerIdType id = 0; id < total; ++id) {
                            ...
                            auto& representatives =
                                admission->stripes[existing.first % 64]->representatives;
        ...
        admission_stripe = node.duplicate_admission_->stripes[key.first % 64].get();
