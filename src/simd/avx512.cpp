...
NormalizeWithCentroid(const float* from, const float* centroid, float* to, uint64_t dim) {
    ...
    if (dim >= 16) {
        __m512 sum = _mm512_setzero_ps();
        for (; i + 15 < static_cast<int>(dim); i += 16) {
        ...
        norm_sq += avx2::FP32ComputeL2Sqr(from + i, centroid + i, dim - i);
    } else {
        norm_sq = avx2::FP32ComputeL2Sqr(from, centroid, dim);
    }
    ...
    for (i = 0; i + 15 < static_cast<int>(dim); i += 16) {
        __m512 f = _mm512_loadu_ps(from + i);
        __m512 c = _mm512_loadu_ps(centroid + i);
        __m512 diff = _mm512_sub_ps(f, c);
...
InverseNormalizeWithCentroid(
    ...
    if (i < static_cast<int>(dim)) {
        avx2::InverseNormalizeWithCentroid(from + i, centroid + i, to + i, dim - i, norm);
    }
#else
