...
ReasoningContext::GenerateReport() const {
    ...
    for (uint8_t value = 0; value < static_cast<uint8_t>(ReasoningDiagnosis::kCount); ++value) {
        const auto d = static_cast<ReasoningDiagnosis>(value);
        {
            JsonType item;
    ...
    for (uint8_t e = 0; e < static_cast<uint8_t>(ReasoningEvent::kCount); ++e) {
...
ReasoningContext::MakeStatusReport(ReasoningReportStatus status, std::string_view index_type) {
    ...
    report["meta"]["index_type"].SetString(
        std::string(capability != nullptr ? capability->index_type : index_type));
    return report.Dump();
}
