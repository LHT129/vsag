...
LogicalExecutor::LogicalExecutor(const ExprPtr& expr, const AttrInvertedInterfacePtr& attr_index)
    : Executor(expr, attr_index) {
    ...
    this->left_ = Executor::MakeInstance(logic_expr->left, attr_index);
    this->right_ = Executor::MakeInstance(logic_expr->right, attr_index);
...
LogicalExecutor::logical_run() {
    if (this->op_ == LogicalOperator::AND) {
        if (this->left_->only_bitset_ and this->right_->only_bitset_) {
            ...
            this->filter_ = std::make_shared<UniqueFilter>(this->bitset_);
        } else {
            this->only_bitset_ = false;
            auto filter_func = [this](int64_t id) -> bool {
                return not(this->left_->filter_->CheckValid(id) and
                           this->right_->filter_->CheckValid(id));
            };
            this->filter_ = std::make_shared<UniqueFilter>(filter_func);
        }
        ...
        if (this->left_->only_bitset_ and this->right_->only_bitset_) {
            ...
            this->only_bitset_ = false;
            auto filter_func = [this](int64_t id) -> bool {
                return not(this->left_->filter_->CheckValid(id) or
                           this->right_->filter_->CheckValid(id));
            };
            this->filter_ = std::make_shared<UniqueFilter>(filter_func);
        }
