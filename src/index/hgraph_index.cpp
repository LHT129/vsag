...
HGraphIndex::hnsw_add(const DatasetPtr& base) {
    ...
    auto build_func = [&](uint64_t begin, uint64_t end) -> void {
...
HGraphIndex::knn_search(const DatasetPtr& query,
    ...
    auto results = this->search_one_graph(
...
HGraphIndex::serialize() const {
    ...
    try {
        ...
        BinarySet bs;
        bs.Set("hgraph", b); // TODO(LHT): rename
...
HGraphIndex::deserialize(StreamReader& reader) {
    ...
    vsag::Vector<std::shared_mutex>(this->GetNumElements(), allocator_).swap(this->neighbors_mutex_);
}
