...
public:
    ...
    Deserialize(const ReaderSet& reader_set) override {
        auto func = [&](uint64_t offset, uint64_t len, void* dest) -> void {
            reader_set.Get("hgraph")->Read(offset, len, dest);
        };
        uint64_t cursor = 0;
...
public:
    ...
    Vector<std::shared_ptr<GraphInterface>> route_graphs_;
...
private:
    ...
    InnerIdType enter_point_id_{std::numeric_limits<InnerIdType>::max()};
