...
public:
    ...
    virtual void
    InsertVector(const float* vector, BucketIdType bucket_id, LabelType label) = 0;

    virtual void
    InsertVector(const float* vector, BucketIdType bucket_id) = 0;
...
public:
    ...
    GetBucketCount() {
        return this->bucket_counts_;
    }

    virtual void
    Serialize(StreamWriter& writer) {
        StreamWriter::WriteObj(writer, this->bucket_counts_);
        StreamWriter::WriteObj(writer, this->code_size_);
    }

    virtual void
    Deserialize(StreamReader& reader) {
        StreamReader::ReadObj(reader, this->bucket_counts_);
        StreamReader::ReadObj(reader, this->code_size_);
    }

public:
    BucketIdType bucket_counts_{0};
