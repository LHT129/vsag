...
void
TestGraphDataCell(const nlohmann::json& graph_param,
                  const nlohmann::json& io_param,
...
TEST_CASE("graph basic test [ut][graph datacell]") {
    ...
    for (auto degree : max_degrees) {
        for (auto capacity : max_capacities) {
            ...
            graph_params.emplace_back(nlohmann::json::parse(str));
        }
    }
